...
\begin{titlepage}
    ...
    \vfill
    {\large \textbf{GitHub Repository:} \url{https://github.com/Rahul09123/Red-Bus}\par}
...
This service owns the transport domain: expeditions, route listing, seat visibility, and ticket-related workflows. It implements a \textbf{Granular Dynamic Pricing Engine} that automatically adjusts ticket prices in real-time based on bus occupancy rates. It is core to customer and company operational flows and can be scaled independently when booking/search load increases.
...
\end{figure}
\textbf{Explanation:} The User Dashboard provides a personalized entry point for customers to manage their profile, view booking history, and access the search module. It demonstrates the platform's role-based authorization where specific features are exposed only to authenticated users, ensuring a secure and streamlined user experience.

\section{Advanced Business Logic Implementation}

To elevate the platform from a simple CRUD application to a production-grade system, several advanced business features were implemented:

\subsection{Granular Dynamic Pricing Engine}

Instead of static pricing, the platform uses a continuous linear growth model to optimize revenue and manage demand. The formula used for every search result is:
\[ \text{Price} = \text{BasePrice} \times (1 + 0.5 \times \text{OccupancyRate}) \]
This ensures that as a bus fills up, the price increases incrementally per seat, rather than in sudden jumps. All financial calculations use \texttt{BigDecimal} with \texttt{HALF\_UP} rounding to ensure mathematical precision for Indian Rupee (₹) transactions.

\subsection{Interactive Route Visualization}

To improve the user experience, the search results page integrates \texttt{React-Leaflet} to provide a geospatial view of the journey. The system maps city names to geographic coordinates (Latitude/Longitude) and draws an interactive dashed polyline between the origin and destination on a high-quality map tile layer.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{images/report/Frontend-having-map.png}
    \caption{Interactive Route Visualization between Source and Destination}
\end{figure}
\textbf{Explanation:} This visualization provides users with a clear geospatial understanding of the journey path. By mapping city coordinates and drawing a dynamic polyline, the platform enhances the booking experience with real-time feedback on the selected route.

\subsection{Real-Time Seat Blocking}

To prevent race conditions and improve concurrent booking reliability, the platform implements a real-time seat blocking mechanism. When a user selects a seat, it is immediately marked as \texttt{BLOCKED} in the database for a duration of 2 minutes. This status is broadcast to all other users viewing the same expedition via a 3-second polling mechanism in the frontend, rendering the seat unavailable (Orange) for them in real-time.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{images/report/Seat_block_feature.png}
    \caption{Real-Time Seat Blocking and Concurrency Control}
\end{figure}
\textbf{Explanation:} This feature ensures data integrity in a high-concurrency environment. By implementing a temporary block at the database level and synchronized UI updates, the system avoids double-booking conflicts and provides immediate visual feedback to all active users.

\subsection{Operator Business Intelligence Dashboard}

The Company (Operator) role features a dedicated dashboard that provides real-time statistics aggregated across all their expeditions. The dashboard tracks:
\begin{itemize}
    \item \textbf{Total Trips}: Number of active and past expeditions.
    \item \textbf{Total Tickets Sold}: Cumulative volume of seat bookings.
    \item \textbf{Net Revenue}: Total earnings calculated in INR.
    \item \textbf{Occupancy Trends}: Business health indicators.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{images/Company_Expedition_list.png}
    \caption{Operator Dashboard with Real-time Business Metrics}
\end{figure}
\textbf{Explanation:} This view demonstrates the backend's ability to perform complex JPA projections and aggregations to provide actionable business intelligence to transport operators.
